...
def test_discovers_tinkoff_broker_file(tmp_path, app_config):
    (tmp_path / "Налоговый отчёт за 2026 год (2026-08-02 21_07_33).xlsx").touch()
    result = discover_sources(tmp_path, app_config)
    assert result.tinkoff_broker is not None
    assert result.tinkoff_broker.name == "Налоговый отчёт за 2026 год (2026-08-02 21_07_33).xlsx"


def test_returns_none_when_tinkoff_broker_missing(tmp_path, app_config):
    result = discover_sources(tmp_path, app_config)
    assert result.tinkoff_broker is None
